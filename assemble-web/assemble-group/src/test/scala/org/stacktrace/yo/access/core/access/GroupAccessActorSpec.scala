...
import org.stacktrace.yo.access.core.access.GroupAccessActor.{GetAccessForUser, GetHostAccessForUser, GroupAccess, GroupTypeAccess}
...
class GroupAccessActorSpec extends AssemblePersistenceSpec(ActorSystem("access-system")) {
  ...
  "A GroupAccessActor" must {
    ...
    //TODO change when guest can add
    "get hosted and guest groups for a single user" in new TestContext() {
      val director = TestProbe()
      val actorTest: ActorRef = system.actorOf(Props(new GroupAccessActor(director.ref)))
      actorTest ! CreateGroupAccess("hostId", "123")
      actorTest ! CreateGroupAccess("hostId", "1234")
    ...
    "get hosted groups for a single user" in new TestContext() {
      ...
      actorTest ! CreateGroupAccess("hostId", "123")
      actorTest ! CreateGroupAccess("hostId", "1234")
      actorTest ! GetHostAccessForUser("hostId")
      val message: GroupTypeAccess = expectMsgType[GroupTypeAccess]

      message.access.size shouldBe 2
      message.aType shouldBe "HOST"
      message.access.contains("123") shouldBe true
      message.access.contains("1234") shouldBe true
    }
  }
